...
class HUD : Module("HUD", Category.HUD) {
    ...
    fun renderHUD() {
        ...
        FutureClient.instance.moduleManager.getModules()
            .filter { it.toggled && it.name != "HUD" && it.name != "CustomMenu" }
        ...
        if (showFPS.value) fr.drawStringWithShadow("FPS: ${Minecraft.getDebugFPS()}", 2f, 2f, -1)
        if (showCPS.value) {
            updateCPS()
            fr.drawStringWithShadow("CPS: ${getCPS()}", 2f, 12f, -1)
        }
        if (showKeystrokes.value) renderKeystrokes(2, 25)
    }
    ...
    private fun updateCPS() {
        if (Mouse.isButtonDown(0)) { /* Hook required for perfect CPS */ }
